...
def load_sunbit_instructions(lang):
    with open(f"{data_dir}languages/{lang}/sunbit-instructions.html", "r") as f:
        sunbit_instructions = f.read()
    return sunbit_instructions
...
def load_languages():
    ...
    sunbit_instructions = load_sunbit_instructions("ens")
    languages = {}
    ...
    for lang in language_codes:
        ...
        languages[lang] = {
            ...
            "language_code": lang,
            "sunbit_instructions": sunbit_instructions
        }
    return languages
...
def index():
    #ROUTE TO /ens
    return redirect(url_for("language", lang_code="ens"))
...
def q():
    #stop the server
    #look for gunicorn processes and kill all of them with 9
    import subprocess
    subprocess.run(["pkill", "-9", "gunicorn"])
    
    return "Server stopped"

@app.route('/api/validate_guess', methods=['POST'])
def validate_guess():
    try:
        print("validate_guess called")
        guess = request.json.get('guess', '').lower()
        language_code = request.json.get('language_code')
        
        # Get the current language and daily word
        word_list = language_codes_5words[language_code]
        language = Language(language_code, word_list)
        daily_word = language.daily_word.lower()
        
        if not guess or len(guess) != 5:
            return jsonify({'error': 'Invalid guess'}), 400
            
        # Create the result array (similar to your current frontend logic)
        result = []
        word_chars = list(daily_word)
        ...
        # Second pass: find misplaced characters
        for i in range(5):
        ...
        return jsonify({
            'result': result,
            'isCorrect': guess.lower() == daily_word.lower(),
            'isValid': guess in language.word_list or guess in language.word_list_supplement
        })
